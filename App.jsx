...
import {
  ...
  Home, Utensils, ListChecks, ChevronLeft, ChevronRight, CalendarDays,
} from "lucide-react";
...
import { GOOGLE_CALENDAR_ID, GOOGLE_CALENDAR_API_KEY } from "./calendarConfig";
  ...
  return (
    <div style={S.app}>
      ...
      <nav style={S.nav}>
        ...
        <NavBtn icon={<CalendarDays size={20} />} label="Calendrier" active={tab === "calendrier"} onClick={() => setTab("calendrier")} />
...
function ProchainsRdv() {
  const [events, setEvents] = useState(null);
  const [error, setError] = useState(null);
  const configured = GOOGLE_CALENDAR_ID !== "REMPLACER_PAR_ID_CALENDRIER" && GOOGLE_CALENDAR_API_KEY !== "REMPLACER_PAR_CLE_API";
  ...
  useEffect(() => {
    ...
    const url =
      ...
      `?key=${GOOGLE_CALENDAR_API_KEY}&timeMin=${encodeURIComponent(timeMin)}&maxResults=10&singleEvents=true&orderBy=startTime`;
    fetch(url)
      .then((r) => r.json())
      .then((data) => {
        if (data.error) {
      ...
      .catch(() => setError("Impossible de contacter Google Calendar."));
  }, [configured]);

  function formatEvent(ev) {
    if (ev.start?.date) {
      const d = new Date(ev.start.date + "T00:00:00");
      return `${JOURS[d.getDay()]} ${d.getDate()} ${MOIS[d.getMonth()].toLowerCase()} — journée entière`;
    }
    const d = new Date(ev.start.dateTime);
    const heure = d.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" });
    return `${JOURS[d.getDay()]} ${d.getDate()} ${MOIS[d.getMonth()].toLowerCase()} — ${heure}`;
  }

  return (
    <div style={{ marginTop: 22 }}>
      <p style={{ fontSize: 13.5, fontWeight: 600, color: "#2B2B26", margin: "0 0 10px" }}>Prochains rendez-vous (calendrier partagé)</p>

      {!configured && (
        <p style={{ fontSize: 13, color: "#9B998F" }}>
          Non configuré — renseignez GOOGLE_CALENDAR_ID et GOOGLE_CALENDAR_API_KEY dans src/calendarConfig.js.
        </p>
      )}
      {configured && error && <p style={{ fontSize: 13, color: "#B85C4A" }}>{error}</p>}
      {configured && !error && events === null && <p style={{ fontSize: 13, color: "#9B998F" }}>Chargement…</p>}
      {configured && events && events.length === 0 && <p style={{ fontSize: 13, color: "#9B998F" }}>Aucun rendez-vous à venir.</p>}

      {configured && events && events.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {events.map((ev) => (
            <div key={ev.id} style={S.gardeListRow}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 13.5, color: "#2B2B26", margin: 0, fontWeight: 500 }}>{ev.summary || "(Sans titre)"}</p>
                <p style={{ fontSize: 12, color: "#9B998F", margin: "2px 0 0", textTransform: "capitalize" }}>{formatEvent(ev)}</p>
              </div>
            </div>
          ))}
        </div>
